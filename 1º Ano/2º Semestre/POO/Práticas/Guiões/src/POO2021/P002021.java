...
public class P002021 {
	...
	private static void alinea2(PrintStream out) {
		...
		try (Scanner sc = new Scanner(new File("src\\POO2021\\uafest.txt"))) {
			String[] header = sc.nextLine().split(";");
			fest = new AgenciaTuristica(header[0], header[1]);
			while (sc.hasNextLine()) {
				String linha[] = sc.nextLine().split(";");
				if (linha[0].equals("1")) {
					fest.add(new Gastronomia(Integer.parseInt(linha[1]), linha[2]));
				} else {
					String[] sitios = new String[linha.length - 3];
					for (int i = 3; i <linha.length; i++) {
						sitios[i - 3] = linha[i];
					}
					fest.add(new PasseioBicicleta(Integer.parseInt(linha[1]), linha[2], sitios));
				}
			}
