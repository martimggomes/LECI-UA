...
Graph* GraphCreateTranspose(const Graph* g) {
  ...
  // Step 1: Create a new graph with the same number of vertices
  Graph* transposedGraph = GraphCreate(g->numVertices, 1, g->isWeighted);

  // Step 2: Iterate through each vertex in the original graph
  List* vertices = g->verticesList;
  ListMoveToHead(vertices);
  for (unsigned int i = 0; i < g->numVertices; ListMoveToNext(vertices), i++) {
    ...
    // Step 3: Iterate through each edge of the vertex
    ListMoveToHead(edges);
    for (unsigned int j = 0; j < ListGetSize(edges); ListMoveToNext(edges), j++) {
      struct _Edge* e = ListGetCurrentItem(edges);
      unsigned int adjVertex = e->adjVertex;

      // Add the reversed edge to the transposed graph
      GraphAddEdge(transposedGraph, adjVertex, v->id);
    }
  }

  // Step 4: Return the transposed graph
